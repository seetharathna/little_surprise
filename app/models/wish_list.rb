...
def WishList.birthday_reminder
  ...
  #user_ids = users.collect{|u| [u.facebook_id, u.session_key]}
    ...
    for user in users
      ...
      puts "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa#{user.facebook_id}"
      begin
        ...
        unless @date.blank?
          ...
          if (@birthday - Date.today >= 1 and @birthday - Date.today <= 30)
             FacebookPublisher.deliver_notification_email(user,fb_user,100000402570887,@birthday)
          end  
        end   
